...
class BackofficeAdministratorsEditController
{
    ...
    public function http_get(array &$params): IView
    {
        if (isset($params['administrators'])) {
            ...
            $title = $translations->get('gestione_amministratori') . ' | ' . $translations->get('nome_sito');

            $id = intval($params['administrators']);
            $administrator = $this->user_repository->get_by_id($id);

            $view_model = new BackOfficeViewModel('backoffice.administrators.edit', $title, $languages, $translations);

...
jQuery(".validate-it").on('click', function (e) {
    ...
    var classToValidate = '.' + jQuery(this).attr("id");
    ...
    jQuery(classToValidate).each(function () {
        ...
        if (jQuery(this).hasClass("nome_categoria")) {
            //Validazione password
            let cat = jQuery(this).val();
            if (cat.length < 1) {
                error_message = "- Il nome della categoria è obbligatorio per tutte le lingue abilitate";
                is_error = true;

                jQuery(this).after('<div class="error_message">' + error_message + '</div>');
            }
        }
        if (jQuery(this).attr("id") == "data_checkin") {
            let cat = jQuery(this).val();
            if (cat == "" || cat == undefined) {
                error_message = "- Scegliere una data per il check-in";
                is_error = true;

                jQuery(this).after('<div class="error_message">' + error_message + '</div>');
            }
        }
        if (jQuery(this).attr("id") == "data_checkout") {
            let cat = jQuery(this).val();
            if (cat == "" || cat == undefined) {
                error_message = "- Scegliere una data per il check-out";
                is_error = true;

                jQuery(this).after('<div class="error_message">' + error_message + '</div>');
            }
        }
        if (jQuery(this).attr("id") == "numero_ospiti") {
            let cat = jQuery(this).val();
            if (cat == "" || cat == undefined) {
                error_message = "- Inserire il numero di ospiti previsti";
                is_error = true;

                jQuery(this).after('<div class="error_message">' + error_message + '</div>');
            }
        }
        if (jQuery(this).attr("id") == "numero_stanza") {
            let cat = jQuery(this).val();
            if (cat == "" || cat == undefined) {
                error_message = "- Inserire il numero della stanza";
                is_error = true;

                jQuery(this).after('<div class="error_message">' + error_message + '</div>');
            }
        }
        if (jQuery(this).attr("id") == 'conferma_password') {
            //Validazione password
            let password = jQuery(this).val();
            if (password.length < 8) {
                error_message = "- La password deve contenere almeno 8 caratteri";
        ...
        if (jQuery(this).attr("id") == 'conferma_password-type-2') {
            //Validazione password
            let password = jQuery(this).val();
            if (password != jQuery("#password-type-2").val()) {
        ...
        if (jQuery(this).attr("id") == 'nome') {
            //Validazione password
            let password = jQuery(this).val();
            if (password.length == 0) {
                error_message = "- Devi inserire un nome";
                is_error = true;
        ...
        if (jQuery(this).attr("id") == 'cognome') {
            //Validazione password
            let password = jQuery(this).val();
            if (password.length == 0) {
                error_message = "- Devi inserire un cognome";
                is_error = true;
        ...
        if (jQuery(this).attr("id") == 'livello') {
            //Validazione password
            let password = jQuery(this).val();
            if (password.length == 0) {
                error_message = "- Inserire un livello valido";
                is_error = true;
        ...
        if (jQuery(this).attr("id") == 'conferma_password') {
            let password = jQuery(this).val();
            let password_first = jQuery("#password").val();
...
jQuery("#creaEvento").click(function () {

    var successo = jQuery(this).attr("data-success");
    var failure = jQuery(this).attr("data-failure");

    if (jQuery("#recupera").is(":checked"))
        var recupera = 1;
    else var recupera = 0;

    if (jQuery("#recupera_convenzione").is(":checked"))
        var recupera_convenzione = 1;
    else var recupera_convenzione = 0;
    ...
    descrizione_evento.push($('#descrizione-evento-<?php echo $i;?>').summernote('code'));
    <?php } ?>
    var is_error = false;
    jQuery(".error_message").remove(); //rimuove tutti gli errori già visualizzati
    ...
    if (is_error == false) {
        ...
        $.post("backoffice/api?use=addEvento", {
            ...
            recupera: recupera,
            recupera_convenzione: recupera_convenzione,
            struttura_associata: strutture_associate,
        ...
        $(".notification-message").addClass("nm-error");
        $(".notification-message").fadeIn();
        $("#creaEvento").after("<span style=\"    color: red;padding: 5px 20px;border: 1px solid;margin: 0 15px;border-radius: 3px;\">Alcuni campi non sono corretti<span>");

    }
});


jQuery("#updateEvento").click(function () {
    ...
    if (is_error == false) {
        ...
        $(".notification-message").addClass("nm-error");
        $(".notification-message").fadeIn();
        $("#updateEvento").after("<span style=\"    color: red;padding: 5px 20px;border: 1px solid;margin: 0 15px;border-radius: 3px;\">Alcuni campi non sono corretti<span>");
